...
export function useBehaviorTracking() {
  ...
  const trackBehavior = useCallback(async (eventData: BehaviorEventData) => {
    if (!session?.user?.id) {
      console.warn('⚠️ No user session found for behavior tracking');
      return;
    }

    try {
      // Call API instead of direct ML infrastructure
      const response = await fetch('/api/ml/track-behavior', {
        method: 'POST',
        headers: {
...
export function withBehaviorTracking<P extends object>(
  ...
  return function TrackedComponent(props: P) {
    ...
    useEffect(() => {
      if (options?.trackClicks && componentRef.current) {
        const handleClick = (event: MouseEvent) => {
          const target = event.target as HTMLElement;
          const elementName = target.dataset.track || target.tagName.toLowerCase();
          tracking.trackClick(elementName, {
    ...
    useEffect(() => {
      if (options?.customEvents) {
        ...
        options.customEvents.forEach(({ eventType, eventName, selector }) => {
          ...
          eventListeners.push({ event: eventType, handler });
        });
    ...
    return React.createElement(Component, { ref: componentRef as any, ...props });
  };
}